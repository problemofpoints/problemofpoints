...
stock_prices <- stock_tickers %>%
  ...
  mutate(year = lubridate::year(date), qrt_ = lubridate::quarter(date))
...
data_to_use %>% 
  summarise(irr = (1+FinCal::irr(cash_flow_final))^4-1,
            irr2 = (1+FinCal::irr(cash_flow_final2))^4-1) %>%
  arrange(desc(irr))
