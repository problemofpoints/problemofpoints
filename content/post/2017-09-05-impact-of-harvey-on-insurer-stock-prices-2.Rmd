...
create_summary_tbl <- function(ci_model){
  ...
  summary_tbl <- ci_model$series %>% as_tibble() %>% last() %>% 
    ...
    mutate(pct_impact = response / point.pred - 1, pct_impact_lower = response / point.pred.lower - 1, 
           pct_impact_upper = response / point.pred.upper - 1) %>% 
