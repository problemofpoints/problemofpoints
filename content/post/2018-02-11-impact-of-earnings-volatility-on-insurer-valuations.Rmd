...
add_sector_ppd_sims <- function(sector, stan_model, draws = 10){
  ...
  vars <- all.vars(terms(stan_model$formula))
  new_roae <- seq(from = 1, to = 30, by = 2.5)
...
ppd_pb_roae <- create_ppd_sims(fit_pb_roae, 4000, "roae_2018","p_to_b") #%>%
  #summarize_ppd_sims("roae_2018","p_to_b", x_lim = c(0, 0.35))

ppd_pb_roae_eps <- create_ppd_sims(fit_pb_roae_eps, 4000, "roae_2018","p_to_b")

gg_ppd <- ppd_pb_roae_eps %>% 
  summarize_ppd_sims("roae_2018","p_to_b", x_lim = c(0.03, 0.15), y_lim = c(0.0, 3.0))
...
ppd_pb_roae$ppd_summary %>% 
  ggplot(aes(y = actual, x = variable / 100)) + 
  geom_linerange(aes(ymin = pctl_25, ymax = pctl_75, x = variable / 100), colour = aon.colors$lightgray) + 
  geom_point() + 
  geom_point(aes(y = median_fitted, x = variable/100), colour = aon.colors$lightgray) + 
  geom_abline(aes(intercept = intercept, slope = roae_2018*100), 
              colour = aon.colors$lightgray, data = pb_roae_params) + 
  facet_wrap(~ sector)
...
loo_fit_pb_roae2 <- loo(fit_pb_roae2)
compare_models(loo_fit_pb_roae, loo_fit_pb_roae2, loo_fit_pb_roae_eps)
