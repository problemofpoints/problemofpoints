...
# Convert to a tidy cumulative DataFrame for the accident years of interest
cum_df = (
    triangle.to_frame()
    .reset_index()
    .rename(columns={"index": "accident_year"})
    .assign(accident_year=lambda df: pd.to_datetime(df["accident_year"]).dt.year)
)
# Accident years 1989-1997 correspond to the R article's summaries
cum_df = cum_df.query("accident_year >= 1989")
latest_paid = cum_df[["accident_year", 12, 24, 36, 48, 60, 72, 84, 96, 108]].set_index("accident_year")
latest_paid
```
...
cum_matrix = cum_df.set_index("accident_year")
incr_matrix = cum_matrix.diff(axis=1)
incr_matrix.iloc[:, 0] = cum_matrix.iloc[:, 0]

incr_long = (
    incr_matrix.reset_index()
    .melt(id_vars="accident_year", var_name="dev", value_name="incremental")
    .assign(dev=lambda df: df["dev"].astype(int))
)
...
future_cells["accident_year"] = future_cells["accident_year"].astype("category")
future_cells["dev"] = future_cells["dev"].astype("category")
...
reserve_summary = reserve_summary.reset_index().rename(columns={"level_0": "accident_year"})

# Add total row and attach the observed latest paid amounts for context
latest_by_year = latest_paid[108].reset_index().rename(columns={108: "latest_paid"})
...
ppc_idata.add_groups(observed_data=obs_ds)
