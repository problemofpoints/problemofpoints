...
line_table_display = (
    line_table.assign(
        r=lambda df: df["phi"].map("{:.2f}".format),
        ci=lambda df: df.apply(
            lambda row: f"[{row['ci_low']:.2f}, {row['ci_high']:.2f}]", axis=1
        ),
        obs=lambda df: df["obs"].astype(int),
    )[["display_name", "obs", "r", "ci"]]
    .rename(
        columns={
...
phi_hat, phi_ci, line_table_display
```
...
for line, df in dev_factors.groupby("line"):
    ce = calendar_effects_from(df.copy())
    for lag in range(1, max_lag + 1):
        aligned = pd.concat([ce, ce.shift(lag)], axis=1, keys=["curr", "lagged"]).dropna()
        if len(aligned) > 1:
            r = aligned["curr"].corr(aligned["lagged"], method="spearman")
        else:
            r = np.nan
        rank_corr_rows.append({"line": line, "lag": lag, "rank_corr": r})

rank_corr_table = pd.DataFrame(rank_corr_rows)
rank_corr_display = (
    rank_corr_table.assign(line=lambda df: df["line"].map(display_names))
    .pivot(index="line", columns="lag", values="rank_corr")
    .pipe(lambda df: df.rename_axis("Line of business").rename(columns=lambda c: f"Lag {c}"))
    .applymap(lambda r: f"{r:.2f}" if pd.notna(r) else "–")
)
